...
/*   Updated: 2025/03/04 13:40:47 by mravelon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pl	*plane(char **str)
{
	...
	if (vect_length(pl->direction) != 1)
		pl->direction = ft_normalize(pl->direction);
	ft_replace_str(str[3], "\n", '\0');
...
t_cy	*cylinder(char **str)
{
	...
	if (vect_length(cy->direction) != 1)
		cy->direction = ft_normalize(cy->direction);
